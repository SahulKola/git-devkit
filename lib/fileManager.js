...
function backup(file) {
  if (fs.existsSync(file)) {
    ...
    // Use path.sep to handle platform-specific separators
...
/**
 * Detects provider from hostname (case-insensitive).
 */
...
export function getExistingSetupInfo() {
  ...
  const gitPath = getGitConfigPath();
  ...
  if (fs.existsSync(gitPath)) {
    ...
    for (const alias of aliases) {
      ...
      if (blockStart !== -1 && blockEnd !== -1) {
        ...
        // Match gitdir path regardless of separators (/ or \)
        const folderMatch = block.match(/\[includeIf "gitdir(?:case)?:(.+?)[\/\\]+"\]/);
        if (folderMatch) {
          accountDetails[alias] = accountDetails[alias] || {};
          // Normalize the folder path from config
          accountDetails[alias].folder = normalizePath(folderMatch[1].trim());
        }
      }
    }
  }
...
export function cleanupAccounts(accountLabels, accountDetails) {
  ...
  const knownHostsPath = path.join(getSshDirectory(), 'known_hosts');
  const knownHostsOldPath = path.join(getSshDirectory(), 'known_hosts.old');
  const removed = [];
  ...
  for (const label of accountLabels) {
    ...
    backup(path.join(getSshDirectory(), `id_ed25519_${label}`));
    backup(path.join(getSshDirectory(), `id_ed25519_${label}.pub`));
  }
  ...
  // Remove SSH keys
  for (const label of accountLabels) {
    const keyPath = path.join(getSshDirectory(), `id_ed25519_${label}`);
